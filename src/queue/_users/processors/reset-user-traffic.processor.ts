...
export class ResetUserTrafficQueueProcessor extends WorkerHost {
    ...
    async process(job: Job) {
        ...
        try {
            switch (job.name) {
                ...
                case USERS_JOB_NAMES.RESET_MONTHLY_CUSTOM_DAY_USER_TRAFFIC:
                    return await this.handleResetUserTraffic(job, RESET_PERIODS.MONTH_CUSTOM_DAY);
    ...
    private async handleResetAllUserTraffic(job: Job) {
        try {
            ...
            await this.handleResetUserTraffic(job, RESET_PERIODS.MONTH_CUSTOM_DAY);
